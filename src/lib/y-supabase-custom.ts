...
export default class SupabaseProvider extends EventEmitter {
	...
	onDocumentUpdate(update: Uint8Array, origin: any) {
		if (origin !== this) {
			...
			console.log('document updated locally, broadcasting update to peers', this.isOnline())
	...
	private async onConnect() {
		...
		console.log('retrieved data from supabase', status)

		if (data && data[this.config.columnName]) {
			// this.logger('applying update to yjs');
			console.log('applying update to yjs')
			try {
		...
		console.log('setting connected flag to true')
		this.isOnline(true)
	...
	constructor(
		...
		if (this.config.resyncInterval || typeof this.config.resyncInterval === 'undefined') {
			...
			this.resyncInterval = setInterval(() => {
				...
				console.log('resyncing (resync interval elapsed)')
	...
	public onAuth(message: Uint8Array) {
		...
		if (!message) {
			...
			console.log(`Permission denied to channel`)
		}
